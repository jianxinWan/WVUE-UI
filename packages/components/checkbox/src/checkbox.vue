<template>
  <div class="functional-select-wrapper" @click.stop="checkboxFocus">
    <label class="display-container multiple-select-container clearfix" :class="(show)?'single-selected-focus':''">
      ...
      <p class="multiple-selected-item" v-for="(item, index) in selectedList" track-by="$index" :key="index">
        {{ item.name }}
    ...
    <div class="options-container" v-show="show">
      ...
      <ul class="options-ul-list">
        ...
        <li v-for="(item,index) in displayOptions" @click.stop.prevent="checkboxSelect(item.id)" :class=" selectedIdList.indexOf(item.id)!=-1?'selected':'' " :key="index">{{ item.name }}</li>
      </ul>
    </div>
  </div>
</template>
